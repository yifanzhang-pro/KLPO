...
python "$KLPO_PATH/scripts/train_molt.py" \
  --recipe r1 --molt-path "$MOLT_PATH" --sync \
...
```bash
python "$KLPO_PATH/scripts/train_molt.py" \
  --recipe qwen_math --molt-path "$MOLT_PATH" --sync \
...
per complete batch. The current backend pin requires `--sync` for this single-turn
launcher; its asynchronous default/options are rejected by the native validation.
`force_sync_mode`, queue depth one, and a drained rollout
...
### Unified single-turn and multi-turn experiments

The math launchers above remain available. [Experiment settings](../examples/README.md)
also provide single-turn R1 and Qwen-Math, Qwen2.5 Python-tool (10 turns), Qwen3
Python-tool (20 turns), and ALFWorld (50 turns) configurations adapted from FlashREINFORCE. Run them with
`examples/run_experiment.py`; all default to token regression + MC-KL and set the
required synchronous schedule automatically. Single-turn math defaults to Molt's
built-in math agent. Multi-turn settings require an explicit compatible agent;
prepared datasets are supplied separately for every setting.

The launch wrapper explicitly forwards `MAX_AGENT_TURNS` and, when set,
`ALFWORLD_DATA` through Ray's runtime environment, alongside Molt's existing
worker variables. The agent must enforce the turn cap. Molt's `StepEnvRunner`
keeps all assistant turns and observations in one trajectory, carrying the
sampler records and action masks into `KLPOLoss`. Do not train on per-turn
fragments using the complete-rollout terminal reward. Stochastic tools also
require checking the assumptions behind the exact sequence-regression identity.
